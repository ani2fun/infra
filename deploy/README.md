...
|---|---|---|
| `apps/` | Application manifests with `base/` + `overlays/{dev,prod}/` per app. Argo CD `Application` objects point at `apps/<name>/overlays/prod/`. | **Yes** |
...
| `dr/` | Disaster-recovery pack: `RUNBOOK.md`, `SNAPSHOT.md`, `gates.md`, `secret-recovery.md`, sealed-secrets backup procedure. | No |
...
- Adding a new app: copy [`apps/dummy-app-template/`](apps/dummy-app-template/),
