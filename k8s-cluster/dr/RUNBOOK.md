...
- [ ] Repo cloned at the snapshot revision:
      ...
      cd infra
      git checkout <SHA from dr/SNAPSHOT.md>
...
```bash
scp -r k8s-cluster/bootstrap/host-prep root@<node>:/root/
ssh root@<node>
cd /root/host-prep
```
...
All A records on `kakde.eu` (per `dr/SNAPSHOT.md`) point to
...
```bash
cp k8s-cluster/bootstrap/wireguard/<node>.wg0.conf.example wg0.conf
...
```bash
ssh root@<node> 'bash -s' < k8s-cluster/bootstrap/k3s/create-k3s-resolv-conf.sh
```

### L3.2 Install K3s server on ms-1

```bash
ssh root@ms-1 'bash -s' < k8s-cluster/bootstrap/k3s/install-server-ms-1.sh
```
...
```bash
ssh root@ms-1 'bash -s' < k8s-cluster/bootstrap/k3s/install-calico.sh
```
...
K3S_TOKEN='<<token from L3.2>>' \
  ...
  < k8s-cluster/bootstrap/k3s/install-agent-<node>.sh
```

### L3.5 Apply node labels and taints

```bash
ssh root@ms-1 'bash -s' < k8s-cluster/bootstrap/k3s/apply-node-placement.sh
```
...
fingerprint recorded in `dr/SNAPSHOT.md` (or your password manager).
...
```bash
ssh root@ms-1 'bash -s' < k8s-cluster/platform/traefik/edge-node-placement.sh
```

### L5.2 Apply Traefik manifests

```bash
ssh root@ms-1 'kubectl apply -f' k8s-cluster/platform/traefik/1-namespace.yaml
ssh root@ms-1 'kubectl apply -f' k8s-cluster/platform/traefik/2-serviceaccount.yaml
ssh root@ms-1 'kubectl apply -f' k8s-cluster/platform/traefik/3-clusterrole.yaml
ssh root@ms-1 'kubectl apply -f' k8s-cluster/platform/traefik/4-clusterrolebinding.yaml
ssh root@ms-1 'kubectl apply -f' k8s-cluster/platform/traefik/5-ingressclass.yaml
ssh root@ms-1 'kubectl apply -f' k8s-cluster/platform/traefik/6-deployment.yaml
ssh root@ms-1 'kubectl apply -f' k8s-cluster/platform/traefik/7-service.yaml
```

(or use `kubectl apply -f k8s-cluster/platform/traefik/` if your kubectl
...
```bash
scp k8s-cluster/platform/traefik/edge-guardrail.sh root@vm-1:/usr/local/sbin/
scp k8s-cluster/platform/traefik/edge-guardrail.service root@vm-1:/etc/systemd/system/
...
```bash
ssh root@ms-1 'bash -s' < k8s-cluster/platform/cert-manager/install-cert-manager.sh
```
...
```bash
ssh root@ms-1 'kubectl apply -f' k8s-cluster/platform/cert-manager/clusterissuer-prod.yaml
ssh root@ms-1 'kubectl apply -f' k8s-cluster/platform/cert-manager/clusterissuer-staging.yaml
```
...
```bash
ssh root@ms-1 'bash -s' < k8s-cluster/platform/argocd/install-argocd.sh
ssh root@ms-1 'bash -s' < k8s-cluster/platform/argocd/configure-argocd.sh
```
...
```bash
ssh root@ms-1 'bash -s' < k8s-cluster/platform/postgresql/label-access.sh
```

Edit `k8s-cluster/platform/postgresql/2-secret.yaml` to substitute the
plaintext from `secret-recovery.md`, then:

```bash
for f in k8s-cluster/platform/postgresql/{1..6}-*.yaml; do
...
```bash
ssh root@ms-1 'kubectl apply -k' k8s-cluster/apps/keycloak/overlays/prod/
```
...
```bash
ssh root@ms-1 'kubectl apply -k' k8s-cluster/apps/whoami/overlays/prod/
```
...
3. Uncomment the resource lines in
   `apps/whoami/base/kustomization.yaml` and
   `apps/whoami/overlays/prod/kustomization.yaml`.
4. `kubectl apply -k k8s-cluster/apps/whoami/overlays/prod/`.
...
git add k8s-cluster/dr/SNAPSHOT-*.md
