...
# k8s-cluster/apps/whoami/overlays/prod/sealedsecret-oauth2-proxy.yaml.
#
...
"$script_dir/rotate-generic-secret.sh" \
  apps \
  whoami-oauth2-proxy \
  "$repo_root/k8s-cluster/apps/whoami/overlays/prod/sealedsecret-oauth2-proxy.yaml" \
  "client-id=$CLIENT_ID" \
  "client-secret=$CLIENT_SECRET" \
  "cookie-secret=$COOKIE_SECRET"

echo "==> sealed secret written to k8s-cluster/apps/whoami/overlays/prod/sealedsecret-oauth2-proxy.yaml"
echo
echo "next steps:"
...
echo "  3. kubectl apply -k k8s-cluster/apps/whoami/overlays/prod/"
